...
Una buena práctica de muestreo es el muestreo aleatorio (random sample), el cual puede ser estratificado o no. Este método se utiliza para evitar sesgos en la elección de la población.
...
Luego, igualamos a cero, colo lo cual se obtiene lo siguiente:

$$ \frac{d ln(f(X_{i};\theta))}{d \theta} = 0 \longrightarrow \theta = \frac{\sum_{i=1}^{n} X_{i}}{n}$$
Es decir, el estímador de máxima verosimilitud de $\theta$ corresponde al promedio a la media de los valores $X_{i}$.
...
En l visión frecuentista los parámetros son constantes pero desconcocidos. Es por esto que el intervalo es el que puede o no incluir el valor real de $\theta$, no que este valor este en el intervalo.
...
**one-sided** corresponde a una hipotesis de mayor o menor, es decir, se calcula la probabilidad de que un valor sea solo mayor o solo menor al esperado. Por otro lado, **two-sided** es una comparación de igualdad o desigualdad, por lo que se calcula la probabilidad de que un cierto valor se encuentre en un intervalo. 

**one-sided** puede ser utilizado en un caso donde una empresa asegura que su producto no sobrepasa cierto peso. 
**one-sided** puede ser utilizado en un caso donde una empresa asegura que su medicamente no altera la presión arterial. 
...
filled.contour(x=mu, y=sigma, z=ll_plot, xlab=expression(mu), 
               plot.title = title(main = "Mapa de calor con los valores mas probables"),
...
filled.contour(x=mu, y=n, z=ll_mean_plot, xlab=expression(mu), 
               plot.title = title(main = "Mapa de calor con los valores mas probables"),
...
plot(x1,y1,col="red",ylim=range(c(y1,y2)),xlim=range(c(x1,x2)))
lines(x2,y2,col="green")
legend(10,0.04,legend=c("y1","y2"), col=c("green","red"),pch=c("-","O"))
```
...
likelihood <- function(param) {
  ...
  lambda = param[1]
  # Definimos la likelihood como la suma logaritmica de la función de densidad
  -sum(dpois(df$Age, lambda, log = TRUE))
}


resultado <- nlminb(objective=likelihood, start=c(10) , lower=c(0), upper=c(40))

# Compara distribución real con la distribución normal de máxima verosimilitud
d <- density(df$Age)
y <- dpois(df$Age, resultado$par[1])
...
likelihood <- function(param) {
  ...
  d_f = param[1]
  # Definimos la likelihood como la suma logaritmica de la función de densidad
  -sum(dchisq(df$Age, d_f, log = TRUE))
}
...
# Compara distribución real con la distribución normal de máxima verosimilitud
d <- density(df$Age)
y <- dchisq(df$Age, resultado$par[1])
...
plot(x1,y1,col="red",ylim=range(c(y1,y2)),xlim=range(c(x1,x2)))
lines(x2,y2,col="green")
legend(-5,0.08,legend=c("y1","y2"), col=c("green","red"),pch=c("-","O"))
