...
> El test de Welsh se utiliza para "castigar" diferencias entre números de conjuntos de test no apareados. Si se utiliase el métono comun (n-1) no influiría que cantidad de datos tiene cada conjunto, siendo esto de extrema relevancia. 
...
> Primero, es importante saber que ambos métodos son equivalentes, la máxima verosimilitud se alcanza solo si se minimiza el error cuadratico medio, y vice versa.

> Para la máxima verosimilitud se asume que el error tiene una distribución normal, pero no se tiene una hipotesis sobre la distribución probabilistica de estos. 

#### **Pregunta 6:**

Explique por qué el test de significancia sobre los parámetros de una regresión lineal se realiza bajo la hipótesis nula $\beta_{H_{0}}=0$.
