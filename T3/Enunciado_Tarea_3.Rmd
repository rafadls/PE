...
> Para el caso tres, este tiene una funcion logaritnica que impide separar a $\beta_{0}$ del resto de las expreciones y tambien impide expresar a la función como $Y = X \beta$.
...
> El test de Welsh se utiliza para, al asignar grados de libertad, "castigar" diferencias entre números de conjuntos de test no apareados. Si se utiliase el métono comun (n-m) no influiría que cantidad de datos tiene cada conjunto, siendo esto de extrema relevancia para casos desiguales. 
...
>La interpretación sería que por cada unidad de la categoría, el resultado final aumenta (o disminuye, si $\beta_{1} \cdot \text{categorica}) < 0$) en un valor de $\beta_{1}$.
...
> Primero, es importante saber que ambos métodos son equivalentes en una regresión lineal, la máxima verosimilitud se alcanza solo si se minimiza el error cuadratico medio, y vice versa. De hecho, por medio del método de máxima veosilitud se obtiene que se debe minimizar el error cuadrático en una regresión, y para esto se asume la distribución del error de esta. 

> En resumen, la minimización del error cuadratico es un resultado de la máxima verosimilitud y de sus hipotesis. Además, es importante recalcar que la hipotesis coresponde a la distribución del error, no de los datos en si.
...
> Asumiendo que el error entre la regresión lineal y los valores reales tiene un distribución gaussiana, se busca la máxima verosimilitud para este, la cual se alcanza cuando la suma de los erroes cuadraticos es mínima. 

> Como consecuencia de esto se tiene que minimizar el error cuadratico para que el ajuste sea optimo y este tenga un error aleatorio (no sistemático).

> Los parámetros "$\beta$", tal cómo sabemos, son los interseptos del plano. Debido a las equivalencias explicadas estos resultan ser valores promedio, diferencias de promedios o valores base. 
...
$$Z-score_{Two-Sample} = \dfrac{(\bar x_2 - \bar x_1) - (\mu_1 - \mu_2)}{\sqrt{\dfrac{\sigma_1^2}{\sqrt n_1}+\dfrac{\sigma_2^2}{\sqrt n_2}}}$$	
